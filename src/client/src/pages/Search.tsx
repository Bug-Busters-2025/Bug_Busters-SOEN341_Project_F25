...
import {
  Calendar, MapPin, Users, Search as SearchIcon, Filter, BookmarkCheck, Bookmark,
} from "lucide-react";
...
export default function Search() {
  ...
  const [registeredEvents, setRegisteredEvents] = useState<EventWithOrganizer[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchPublic() {
      ...
      const visible = data.filter(
        (e: any) => (e.status ?? "published").toString().toLowerCase() !== "archived"
      );
      if (!cancelled) setEvents(visible);
    }

    async function fetchUserLists(uid: number) {
      const [savedRes, registeredRes] = await Promise.all([
        axios.get(`${API_BASE}/events/${uid}`),
        axios.get(`${API_BASE}/events/registered/${uid}`),
      ]);
      if (!cancelled) {
        setSavedEvents(Array.isArray(savedRes.data) ? savedRes.data : []);
  ...
  const filteredEvents = useMemo(() => {
    let filtered = events.filter((event) => {
      if (selectedDate) {
        ...
        if (eventDateOnly !== selectedDate) return false;
      }
      if (selectedCategory !== "All" && event.category !== selectedCategory) return false;
      if (selectedOrganization !== "All" && event.organizer_name !== selectedOrganization) return false;
    ...
    return filtered.sort((a, b) => {
      const aReg = registeredEvents.some((e) => e.id === a.id);
      const bReg = registeredEvents.some((e) => e.id === b.id);
  ...
  const handleSaveEvent = async (event: EventWithOrganizer) => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/events/save`, {
        user_id: userId,
        event_id: event.id,
      });
  ...
  const handleDeleteEvent = async (event: EventWithOrganizer) => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/events/unsave`, {
        user_id: userId,
        event_id: event.id,
      });
      if (response.status === 200) {
  ...
  const handleRegisterEvent = async (event: EventWithOrganizer) => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/events/register`, {
        user_id: userId,
        event_id: event.id,
      });
      if (response.status === 200) {
        ...
        setEvents((prev) =>
          prev.map((e) =>
            e.id === event.id ? { ...e, remaining_tickets: e.remaining_tickets - 1 } : e
          )
        );
      }
    } catch (error: any) {
  ...
  const handleUnregisterEvent = async (event: EventWithOrganizer) => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/events/unregister`, {
        user_id: userId,
        event_id: event.id,
      });
      if (response.status === 200) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 animate-in fade-in-0 slide-in-from-top-4 duration-700">
          ...
          <p className="text-lg text-muted-foreground">
            Find events that interest you and connect with your campus community
          </p>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <DatePicker value={selectedDate} onChange={(date) => setSelectedDate(date || "")} placeholder="Select event date" />
              </div>
        ...
        <div className="mb-4 flex items-center gap-2">
          <Button onClick={() => setShowRegisteredOnly(!showRegisteredOnly)} variant={showRegisteredOnly ? "primary" : "outline"}>
            <BookmarkCheck className="h-4 w-4" />
            <span className="text-secondary-foreground">
              {showRegisteredOnly ? "Showing Registered Events" : "Show Registered Events"}
            </span>
          </Button>
          {registeredEvents.length > 0 && (
            <span className="text-muted-foreground">({registeredEvents.length} registered)</span>
          )}
        </div>
        ...
        <div className="mb-6 animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
          <div className="flex items-center gap-2">
            <p>
              <span className="font-semibold text-primary">{filteredEvents.length}</span> event
              {filteredEvents.length !== 1 ? "s" : ""} found
              {(selectedCategory !== "All" || selectedOrganization !== "All" || selectedDate || searchQuery) && (
        ...
        {filteredEvents.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredEvents.map((event, index) => {
              const isSaved = savedEvents.some((s) => s.id === event.id);
              const isRegistered = registeredEvents.some((r) => r.id === event.id);
              return (
                <Card
                  key={event.id}
                ...
                >
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <button
                      onClick={(e) => {
                        if (isSaved) handleDeleteEvent(event);
                        else handleSaveEvent(event);
                        e.stopPropagation();
                      }}
                      className={`absolute top-3 left-3 z-10 p-2 rounded-full backdrop-blur-sm transition-all cursor-pointer ${
                        isSaved ? "bg-primary text-primary-foreground hover:bg-primary/90" : "bg-background/80 text-muted-foreground hover:bg-background hover:text-foreground"
                      } `}
                    >
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                      <CardTitle className="text-lg text-foreground drop-shadow-md">{event.title}</CardTitle>
                    </div>
                  </div>
                    ...
                    {role === "student" && (
                      <>
                        {isRegistered ? (
                          ...
                          >
                            {event.remaining_tickets > 0 ? "Register Now" : "Event Full"}
                          </Button>
                        )}
                      </>
                    )}
