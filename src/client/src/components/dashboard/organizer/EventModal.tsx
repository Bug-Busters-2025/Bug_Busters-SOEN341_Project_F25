   ...
   const handleSubmit = async () => {
      ...
      if (isEditMode && eventData?.event_date) {
         ...
         const oneDayBefore = new Date(
            originalEventDate.getTime() - 24 * 60 * 60 * 1000
         );
         if (now >= oneDayBefore) {
      ...
      try {
         if (isEditMode && eventData?.id) {
            ...
            if (newCapacity < sold) {
               setMessage(
                  `Capacity cannot be lower than ${sold} (tickets already sold).`
               );
               return;
            }
            await axios.put(
   ...
   return (
      <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
         <div className="bg-background p-6 rounded-xl shadow-lg w-[500px] border relative">
            <Button
               onClick={onClose}
               className="absolute top-3 right-3 rounded-full hover:bg-accent/50"
            >
